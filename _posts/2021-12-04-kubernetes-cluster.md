...
![gcp_project.png](/assets/images/belugaBook/ch06/gcp_project.png)
### Step3. GCP SDK를 설치한다. 
...
![sdk_select_project.png](/assets/images/belugaBook/ch06/sdk_select_project.png)
### Step5. 쿠버네티스 클러스터 생성
...
- 책에서 사용한 옵션들 
    1. ```--cluster-version``` : 쿠버네티스의 클러스터 버전으로  책보다 내 클러스터 default 버전이 높기 때문에 클러스터 버전은 지정하지 않고 default를 사용한다.
       ...
       ![default_and_available_cluster_version.png](/assets/images/belugaBook/ch06/default_and_available_cluster_version.png)
    ...
    3. ```--machine-type``` : 노드에서 사용할 컴퓨팅 머신을 지정하는데, default는 e2-medium 이다. 
       ...
       - 머신 종류 ```gcloud compute machine-types list```를 사용하면 너무 길어서 파일에 저장하여 봄. 
         ![gcp_compute_machine.png](/assets/images/belugaBook/ch06/gcp_compute_machine.png)
         ![gcp_compute_machine_list.png](/assets/images/belugaBook/ch06/gcp_compute_machine_list.png)
- 클러스터 생성 :
    1. 전제 : 
        ...
        3. Kubernetes Engine API [활성화](https://console.cloud.google.com/apis/library/container.googleapis.com?project=dooroomi-workbook)
         ![enable_kubernetes_engine_api.png](/assets/images/belugaBook/ch06/enable_kubernetes_engine_api.png)
        4. 클러스터 생성 (시간 좀 걸림)
           ```gcloud container clusters create dooroomi --machine-type=e2-micro --num-nodes=3```
           ![creating_clusters.png](/assets/images/belugaBook/ch06/creating_clusters.png)
        5. 생성된 클러스터 조회하기 
           ```gcloud container clusters describe dooroomi```
           ![read_cluster.png](/assets/images/belugaBook/ch06/read_cluster.png)
           ![read_cluster_on_console.png](/assets/images/belugaBook/ch06/read_cluster_on_console.png)
        6. gcloud로 클러스터를 제어할 수 있도록 kubectl에 인증정보 설정
           ```gcloud container clusters get-credentials dooroomi```
           ![kubectl_gcloud.png](/assets/images/belugaBook/ch06/kubectl_gcloud.png)
        7. 생성한 노드 검색
           ```kubectl get nodes```
           ![kubectl_get_nodes.png](/assets/images/belugaBook/ch06/kubectl_get_nodes.png)
        8. 쿠버네티스 API에 대한 프록시를 생성하면 새로 만든 클러스터의 대시보드도 볼 수 있다. (optional)
            ```kubectl proxy```
           ![kubectl_proxy.png](/assets/images/belugaBook/ch06/kubectl_proxy.png)
        ...
        **kubectx 와 kubens (https://github.com/ahmetb/kubectx)**
         1. kubectx : context(cluster) 스위칭을 간편하게 할 수 있게 도와줌
            ...
            ![kubectx.png](/assets/images/belugaBook/ch06/kubectx.png)
    ...
    3. 스토리지클래스(StorageClass)  
        ...
        ```yaml
         ...
         parameters:
           type: pd-ssd
        ```     
        ![create_storage_class.png](/assets/images/belugaBook/ch06/create_storage_class.png)
...
결정할 것을 이야기 하고 있다. 컨테이너 기술을 사용하였을 경우 전통적인 방법보다는 데이터 유실이 일어날 가능성이 더 높다는 것을 배제할 수 없음을 강조하고 있다. 
  ...
  ![database_decision_tree.png](/assets/images/belugaBook/ch06/database_decision_tree.png)
